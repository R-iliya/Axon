# 🧠 Axon Parser — Full Documentation

> **Module:** `axon/parser.py`
> **Purpose:** Translates tokenized Axon source code into an Abstract Syntax Tree (AST).
> **Depends on:**
>
> * `axon.lexer` → for tokenization
> * `axon.nodes` → for AST node definitions

---

## Overview

The **Axon Parser** is the brain of the language — it takes raw tokens generated by the **Lexer** and builds a structured, hierarchical **Abstract Syntax Tree (AST)** that represents the logical structure of your program.
...
For example, `print(5+3);` → becomes a `PrintNode` containing a `BinOpNode` of two `NumberNode`s.
...
It walks through the token stream and constructs AST nodes according to Axon’s grammar.
...
It’s raised whenever something unexpected happens — like a missing parenthesis, colon, or malformed expression.
...
Every Axon statement ends with a **semicolon (`;`)** — except control blocks (`if`, `for`, etc.) which use `{}`.
...
→ `PrintNode(StringNode("Hello World"))`
...
→ `IfNode(condition, true_body, false_body)`
...
→ `ForNode(var_name, start_expr, end_expr, body)`
...
→ `FunctionNode(name="greet", params=["name"], body=[...])`
...
→ `CallNode("greet", [StringNode("Axon")])`
